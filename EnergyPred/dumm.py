...
# model = LSTM(xs_train.size()[1],xm_train.size()[1],xl_train.size()[1], hidden_dim=hidden_dim, output_dim=output_dim, num_layers=num_layers)
...
for t in range(num_epochs):
    ...
    # Forward pass
    ym_train_pred = model(xs_train, xm_train, xl_train)

    loss = loss_fn(ym_train_pred, ym_train)
    if t % 10 == 0 and t != 0:
