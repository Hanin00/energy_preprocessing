...
def DailyPredict(lstmPth, arimaPth,predS, predE,testYs_tensor , testXs_tensor, testXm_tensor, testXl_tensor, resultDf,ARTrainset ) :
    ...
    num_layers = 2
    output_dim = 1
    inputXm_dim = testXm_tensor.size()[1]
    inputXl_dim = testXl_tensor.size()[1]
    loss_fn = torch.nn.MSELoss()
...
newDf = DailyPredict(lstmPth, arimaPth,predS, predE, testYs, testXs, testXm, testXl,  resultDf, ARTrainset)
print(newDf.head())
print(newDf.iloc[0])
