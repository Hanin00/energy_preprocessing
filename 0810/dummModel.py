...
# todo 일단 predict 코드를 만들어 보자.. 성능은 나중에.. 글고 아무리 생각해도 지금 normalize가 문제인 것 같은데.. 함 봐봅시다..
...
def datasetXsml2(df, seq_length, trainS, trainE, testS ):
    #todo 날짜로 인덱싱
    scaler = MinMaxScaler()
    df["power_value"] = scaler.fit_transform(df["power_value"].values.reshape(-1, 1))
    df = df["power_value"]
...
ARTrainset,ARTestset, trainXs_tensor, trainYs_tensor, testXs_tensor, testYs_tensor, trainDatasetXs, testDatasetXs = datasetXsml2(resultDf, 1,  trainS, trainE, testS )
_, _, trainXm_tensor, trainYm_tensor, testXm_tensor, testYm_tensor, trainDatasetXm, testDatasetXm = datasetXsml2(resultDf, 7, trainS, trainE, testS )
...
# ARTrainset = ARTrainset.loc['2019-01-01' : '2021-08-31']
...
def testDataTrimming(testXs_tensor, testXm_tensor, testXl_tensor) :
    ...
    return testXs_tensor, testXm_tensor, testXl_tensor
