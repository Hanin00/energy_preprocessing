...
class APTN(BaseModel):
    ...
    def __init__(self, model_config):
        ...
        # config setting
        self.config = model_config # {'name': 'APTN', 'T': 24, 'n': 7, 'D': 307, 'horizon': 12, 'm': 128, 'v': 128, 'loss_function': 'mse'}
        self.T = model_config.get('T') # short term 24(5*24=120분,2시간 )  # #previous times lots that our model utilized

        # print(self.T)  #24
        # print(type(self.T)) #type

        self.n = model_config.get('n') # 7   #periods
        self.D = model_config.get('D') # 307    # detectors <-  #node
        self.m = model_config.get('m') # 128            # dimension of the hidden representation of all LSTM unit
        self.v = model_config.get('v') # output dim size 128   @dimension of the feature representaion of the entire graph
        self.horizon = model_config.get('horizon') # 12    <- the average prediction results of traffic volume over the next on houre(K = 12) p -6
        self.loss_tf = get_tf_loss_function(model_config.get('loss_function')) # < function mse_tf at 0x00000277BCB5D1F0 >
        self._hw = self.T // 2 # 12
        ...
        # print("self.loss_tf : ",self.loss_tf)
        # print("self._hw : ", self._hw)
        ...
        with tf.compat.v1.name_scope('inputs'):
            # train placeholder
            self.dropout_rate_ph = tf.compat.v1.placeholder(self.float_dtype, name='dropout_rate') #Tensor("inputs/dropout_rate:0", dtype=float32)
            self.lr_ph = tf.compat.v1.placeholder(self.float_dtype, name='lr')  # Tensor("inputs/lr:0", dtype=float32)
            # data placeholder
            self.X_skip_ph = tf.compat.v1.placeholder(self.float_dtype,shape=[None, self.n, self.T, self.D])
            #Tensor("inputs/Placeholder:0", shape=(None, 7, 24, 307), dtype=float32)   #long term 7일,  2시간, nodes
            self.X_ph = tf.compat.v1.placeholder(self.float_dtype, shape=[None, self.T, self.D], name='X') # Tensor("inputs/X:0", shape=(None, 24, 307), dtype=float32)
            self.Y_ph = tf.compat.v1.placeholder(self.float_dtype, shape=[None, self.horizon, self.D], name='Y') # Tensor("inputs/Y:0", shape=(None, 12, 307), dtype=float32)


        # ------------ model body building -----------
        with tf.compat.v1.variable_scope(self.__class__.__name__):
            # input layer
            # shape -> [batch_size, T, v] #short term
            x_input = self.input_layer(self.X_ph) #<bound method APTN.input_layer of <models.reAPTN.APTN object at 0x0000017C08E36AC0>>
            #x_input :  Tensor("APTN/input_layer/Relu:0", shape=(None, 24, 128), dtype=float32)
            # type(x_input) : <class 'tensorflow.python.framework.ops.Tensor'>
            # x_input.shape :  (None, 24, 128)

            #print("x_input : ", x_input)  Tensor("APTN/input_layer/Relu:0", shape=(None, 24, 128), dtype=float32)
            # shape -> [batch_size, n, T, v] #Long term
            x_skip_input = self.input_layer(self.X_skip_ph) #mid term,
            x_skip_input = self.input_layer(self.X_skip_ph) #long term  Tensor("inputs/Placeholder:0", shape=(None, 7, 24, 307), dtype=float32)

            # hop-rnn layer
            # shape -> [batch_size, T, m]
            period_outputs = self.hop_rnn_layer(x_skip_input) # Tensor("APTN/hop_rnn/transpose:0", shape=(None, 24, 128), dtype=float32)

           #  -> en/decoder 없이 concat
            # encoder and decoder layer
            # shape -> [batch_size, T, m]
            en_outputs = self.att_encoder_layer(x_input, period_outputs) # Tensor("APTN/att_encoder/transpose_1:0", shape=(None, 24, 128), dtype=float32)
            # shape -> [batch_size, m]
            de_output = self.att_decoder_layer(en_outputs, x_input)  #  Tensor("APTN/att_decoder/dropout_23/Mul:0", shape=(None, 128), dtype=float32)
            # shape -> [batch_size, v] #Concat
            output_state = self.att_output_layer(en_outputs[:, -1], de_output) # Tensor("APTN/att_output/add_1:0", shape=(None, 128), dtype=float32)

            # generate predictions
            pred_ta = tf.TensorArray(self.float_dtype, size=self.horizon) #<tensorflow.python.ops.tensor_array_ops.TensorArray object at 0x0000017C0D6BDEE0>
            # pred_ta :  <tensorflow.python.ops.tensor_array_ops.TensorArray object at 0x0000017C0D6BDEE0>

            # shape -> [batch_size, hw, D]
            ar_input = self.X_ph[:, -self._hw:]

            # object at  0x0000017C0D6BDEE0 >
            # self.X_ph: Tensor("inputs/X:0", shape=(None, 24, 307), dtype=float32)
            # self._hw: 12
            # -self._hw: -12
            # ar_input: Tensor("APTN/strided_slice_1:0", shape=(None, 12, 307), dtype=float32)
        ...
        self.Y_pred = tf.transpose(a=pred_ta.stack(), perm=[1, 0, 2])
        ...
        with tf.compat.v1.name_scope('train'):
            ...
            self.optimizer = tf.compat.v1.train.AdamOptimizer(self.lr_ph)
    ...
    def get_rnn_cell(self, rnn_hid, dropout_rate):
        ...
        rnn = tf.compat.v1.nn.rnn_cell.LSTMCell(rnn_hid, initializer=tf.compat.v1.initializers.orthogonal(),
                                      activation=tf.tanh, forget_bias=0.0)
        rnn = tf.compat.v1.nn.rnn_cell.DropoutWrapper(rnn, output_keep_prob=keep_prob)
        return rnn
    ...
    def hop_rnn_layer(self, x, scope='hop_rnn'):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            for t in range(self.T):
                # init cell state
                s_states = skip_rnn.zero_state(tf.shape(input=x)[0], tf.float32)
                h_state = None
            ...
            # <batch_size, T, m>
            skip_outputs = tf.transpose(a=skip_h_states.stack(), perm=[1, 0, 2])
    ...
    def att_encoder_layer(self, en_x, inputs_from_skip, scope='att_encoder'):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            en_input = tf.transpose(a=en_x, perm=[0, 2, 1])
            ...
            en_s_state = en_rnn.zero_state(tf.shape(input=en_input)[0], tf.float32)
            ...
            # <batch_size, T, m>
            en_outputs = tf.transpose(a=x_embeds.stack(), perm=[1, 0, 2])
    ...
    def att_decoder_layer(self, inputs_from_en, de_input, scope='att_decoder'):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            de_s_state = de_rnn.zero_state(tf.shape(input=inputs_from_en)[0], tf.float32)
            de_h_state = None
            ...
            for t in range(self.T):
                ...
                # <batch_size, m>
                c_t = tf.reduce_sum(input_tensor=c_t, axis=-2)
    ...
    def att_output_layer(self, input_from_en, input_from_de, scope='att_output'):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            infer_output = tf.nn.dropout(infer_output, rate=1 - (1 - self.dropout_rate_ph))
            # shape -> [batch_size, v]
