...
'''
기존 텐서플로우로 구현되어있던 코드에서 node 수(detector)만 제외하고 개수 변경, 그리고 일단 기존 전처리된 파일인 npz 파일의 형태를 확인해야 함

10분 단위로 기록된 데이터 target 102_10T_diff.csv 를 사용할 거고 여기서 사용 하는 건 power_value 만 사용할 것. pw_diff는 결측치가 너무 많음 

'''

class tfAPTN(nn.Module):
    def __init__(self, input_dim):
        ...
        self.n = 7  # number of period    일주일 단위로 총 몇 개 있는지. 4
        self.D = 1  # detectors <- node
        self.m = 128   # 128 dimension of the hidden representation of all LSTM unit
        self.v = 128   # output dim simze 128
        ...
        with tf.name_scope('inputs'):
            ...
            self.X_ph = tf.placeholder(self.float_dtype, shape=[None, self.T, self.D], name='X')  # none, 12, 1 인지 확인 필요.
