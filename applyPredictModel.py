...
path = './data/target102_10T_diff.csv'
dataXs = pd.read_csv(path, delimiter=',')
path = './data/target102_1D_diff.csv'   #<- 일자별 max 값으로 들어가야 함. 누적값 말고 증가량이 들어가야 할 듯? 그런데 아예 0인 값들도 있는데 이걸 어카냔 말임... 아예 없는거 아니면 exp7 이런거 넣어야 하나?
#todo : ? - window 가 30, 90 으로 들어가면 되는건가? LSTM  에서? 그러면 데이터 다시 뽑아야 할 듯 - 일자별로~
dataXm = pd.read_csv(path, delimiter=',')


# 7일간의 데이터가 입력으로 들어가고 batch size는 임의로 지정
xs_Seq_length = 7 # 하루에 총 몇 개의 데이터가 들어가는 지 확인해야함
xm_Seq_length = 30 # 한 달에 총 몇 개의 데이터가 들어가는 지 확인해야함
xl_Seq_length = 90 # 세 달에 총 몇 개의 데이터가 들어가는 지 확인해야함
